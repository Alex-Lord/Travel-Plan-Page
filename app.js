...
function xiaohongshuSourcesMarkup(item) {
  const sources = Array.isArray(item.xiaohongshuSources) ? item.xiaohongshuSources : [];
  const links = sources.map((source, index) => {
    ...
    const label = typeof source === "object" && source?.label
      ? source.label
      : `小红书帖子 ${index + 1}`;
    return `<a class="schedule-source-link" href="${escapeHtml(url)}" target="_blank" rel="noopener noreferrer" aria-label="打开对应的${escapeHtml(label)}">📕 ${escapeHtml(label)} ↗</a>`;
  }).filter(Boolean);

  return `
    <div class="schedule-sources${links.length ? "" : " is-empty"}">
      <span class="schedule-sources__label">Excel 对应小红书来源</span>
      ${links.length
        ? `<div class="schedule-source-links">${links.join("")}</div>`
        : `<span class="schedule-sources__empty">Excel 未提供小红书来源</span>`}
    </div>`;
}
...
function dayCard(day) {
  ...
  const schedule = day.schedule.map((item) => {
    ...
    const xiaohongshuSources = xiaohongshuSourcesMarkup(item);
    return `
...
async function decryptTripData(password) {
  if (!globalThis.crypto?.subtle || !globalThis.TextEncoder || !globalThis.TextDecoder) {
    throw new Error("Encrypted travel data is not supported by this browser");
  }
  ...
  if (envelope?.version !== 1 || envelope?.algorithm !== "AES-GCM" || envelope?.kdf !== "PBKDF2-SHA-256") {
    throw new Error("Invalid encrypted travel data");
  }
...
function unlockTravelPage(data) {
...
function setupAccessGate() {
  ...
  if (!gate || !form || !input || !status || !submit) return;

  setProtectedContentLocked(true);
  document.title = "私人旅行计划 · 需要密码";
  ...
  form.addEventListener("submit", async (event) => {
    ...
    try {
      ...
      input.value = "";
      unlockTravelPage(data);
      return;
    } catch (error) {
      const unsupported = error?.message === "Encrypted travel data is not supported by this browser";
      const wrongPassword = error?.name === "OperationError";
      if (!wrongPassword) console.error("Travel data could not be decrypted", error);
      status.textContent = unsupported
        ? "当前浏览器无法验证密码，请更换现代浏览器后重试。"
        : "密码不正确，请重试。";
      input.value = "";
      input.setAttribute("aria-invalid", "true");
      input.focus();
      ...
      submit.textContent = "进入旅行计划";
    }
  });
}

document.addEventListener("DOMContentLoaded", setupAccessGate);
