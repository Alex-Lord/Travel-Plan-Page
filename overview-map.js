...
function travelOverviewArtwork(days, source = {}, options = {}) {
  ...
  ${source.customArtwork ? `<g id="artwork-leaders" stroke="#52665c" stroke-width="1" opacity=".7">${placeLayers.map((place) => `<path id="leader-${esc(place.id)}" d="M${place.x} ${place.y} L${place.tx} ${place.ty - 8}"/>`).join("")}</g>` : ""}
