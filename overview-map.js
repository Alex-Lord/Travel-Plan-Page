...
function travelOverviewArtwork(days, source = {}, options = {}) {
  ...
  ${(source.artworkPanels || []).map((panel) => `<g class="map-artwork-panel"><rect x="${panel.x}" y="${panel.y}" width="${panel.width}" height="${panel.height}" rx="18" fill="#faf9ef" fill-opacity=".97" stroke="#638071" stroke-width="2"/><text x="${panel.x + 24}" y="${panel.y + 38}" font-size="24" fill="#092653">${esc(panel.title)}</text><text x="${panel.x + 24}" y="${panel.y + 68}" font-size="18" fill="#52665c">${esc(panel.subtitle)}</text></g>`).join("")}
  ${source.customArtwork ? '<rect x="18" y="45" width="475" height="410" rx="18" fill="#faf9ef" fill-opacity=".9"/><text x="35" y="435" fill="#52665c" font-size="18">行程示意 · 非导航地图</text>' : ""}
  ...
  ${source.customArtwork ? `<g id="artwork-leaders" stroke="#52665c" stroke-width="1" opacity=".7">${placeLayers.map((place) => `<path id="leader-${esc(place.id)}" data-map-leader="" data-point-x="${place.x}" data-point-y="${place.y}" data-label-x="${place.tx}" data-label-y="${place.ty}" d="M${place.x} ${place.y} L${place.tx} ${place.ty - 8}"/>`).join("")}</g>` : ""}
  <g id="overview-place-names" inkscape:groupmode="layer" inkscape:label="地点名称" fill="#092653" stroke="${source.customArtwork ? "#faf9ef" : "#092653"}" stroke-width="${source.customArtwork ? 4 : .4}" stroke-linejoin="round" paint-order="stroke fill" font-family="'Times New Roman', 'Kaiti SC', STKaiti, KaiTi, 'Songti SC', serif" font-weight="700">${placeLayers.map((place) => `<text id="overview-label-${esc(place.id)}" x="${place.tx}" y="${place.ty}" text-anchor="${esc(place.anchor || "start")}" font-size="${place.size}">${multilineText(place.lines, place.tx)}</text>`).join("")}</g>
